...
from database import UsersClass, PostsClass
...
from kivymd.uix.list import ThreeLineAvatarListItem, TwoLineAvatarListItem, ImageLeftWidget
...
class LoginScreen(Screen):
    ...
    def verifyUser(self):
        ...
        if usersDB.verifyUserExist(self.useremail.text) == True:
            ...
            if self.userpass.text == usersDB.getAccountData(userlogged, 'password'):
                name = usersDB.getAccountData(userlogged, 'firstname') + usersDB.getAccountData(userlogged, 'lastname')
    ...
    def updatePostWall(self):
        ...
        postwallwidgets = []
        results = postsDB.getposts()
        for post in results:
            occupation = usersDB.getAccountData(post['email'], 'profession')
            firstname = usersDB.getAccountData(post['email'], 'firstname')
            lastname = usersDB.getAccountData(post['email'], 'lastname')
            primary_text = firstname + " " + lastname + " (" + occupation + ")"
            secondary_text = post['post']
            ...
            underline = MDLabel(text = "______________________________________________________", halign = "center", size_hint_y = 0.3)
...
class LocationMapViewScreen(Screen):
    ...
    def loadDoctors(self):
        ...
        for doctors in sorted_list:
            ...
            primary_text = usersDB.getAccountData(doctor_email, 'firstname') + " " + usersDB.getAccountData(doctor_email, 'lastname')
            secondary_text = usersDB.getAccountData(doctor_email, 'firstname')
        ...
        user_location = eval(usersDB.getAccountData(userlogged, 'location'))
